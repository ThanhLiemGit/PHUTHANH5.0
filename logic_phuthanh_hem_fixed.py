...
# Trả lại phần cuối số nhà (VD: 158/51C → "51C")
def extract_main_number(so_nha_raw):
    parts = so_nha_raw.upper().split("/")
    last = parts[-1]
    match = re.match(r"(\d+)([A-Z]*)", last)
    if match:
...
def check_address(input_text):
    ...
    for segment in DATA[duong]:
        tu = str(segment["tu"]).upper()
        den = str(segment["den"]).upper()
        if not (tu <= so_chinh <= den):
            continue

        side = segment.get("side", "both")
        if side == "odd" and re.match(r"\d+", so_chinh) and int(re.match(r"\d+", so_chinh).group()) % 2 == 0:
            continue
        if side == "even" and re.match(r"\d+", so_chinh) and int(re.match(r"\d+", so_chinh).group()) % 2 != 0:
            continue
